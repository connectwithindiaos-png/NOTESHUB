...
import { fetchAdminPYQs, fetchAdminCourses, fetchAdminSemesters, fetchAdminSubjects, createNote, updateNote, deleteNote } from '../../utils/api'
import ConfirmModal from '../../components/admin/ConfirmModal'
import toast from 'react-hot-toast'

const emptyForm = {
  courseSlug: '', semesterId: '', subjectId: '', title: '', description: '', driveUrl: '',
...
export default function AdminPYQs() {
  ...
  const load = () => {
    ...
    fetchAdminPYQs(page)
      ...
      .catch(err => toast.error('Failed to load PYQs: ' + err.message))
  ...
  const confirmDelete = async () => {
    try {
      ...
      if (pyqs.length <= 1 && page > 1) setPage(p => p - 1)
      else load()
    } catch (err) {
      toast.error(err.message)
    }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-serif text-2xl text-stone-900 dark:text-stone-100">PYQs</h1>
          <p className="font-sans text-sm text-stone-500 dark:text-stone-400 mt-1">Manage Previous Year Questions</p>
        </div>
        <button
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white dark:bg-stone-800 rounded-2xl border border-stone-200 dark:border-stone-700 p-6 mb-6 shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            <span className="w-8 h-8 rounded-lg bg-rose-100 dark:bg-rose-900/40 flex items-center justify-center text-sm text-rose-700 dark:text-rose-300 font-mono font-bold">
              {editing ? '✎' : '+'}
            </span>
            <h2 className="font-mono text-sm font-semibold text-stone-700 dark:text-stone-300">
              {editing ? 'Edit PYQ' : 'New PYQ'}
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <input type="text" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="w-full px-3 py-2.5 bg-stone-50 dark:bg-stone-700 border border-stone-200 dark:border-stone-600 rounded-xl text-sm text-stone-900 dark:text-stone-100 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-all" placeholder="e.g. 2023 Exam Paper" required />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })} className="w-full px-3 py-2.5 bg-stone-50 dark:bg-stone-700 border border-stone-200 dark:border-stone-600 rounded-xl text-sm text-stone-900 dark:text-stone-100 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-all" placeholder="2023, midterm, important" />
            </div>
          ...
          <div className="mb-4">
            ...
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="w-full px-3 py-2.5 bg-stone-50 dark:bg-stone-700 border border-stone-200 dark:border-stone-600 rounded-xl text-sm text-stone-900 dark:text-stone-100 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-all h-20 resize-none" placeholder="Exam year, subject code, etc." />
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-stone-200 dark:border-stone-700 bg-stone-50/50 dark:bg-stone-700/30">
                  ...
                  <th className="text-left px-4 py-3.5 font-mono text-xs text-stone-500 uppercase tracking-wider">Downloads</th>
                ...
                {(!pyqs || pyqs.length === 0) && (
                  <tr>
                    <td colSpan="5" className="px-4 py-12 text-center">
                      ...
                      <p className="text-stone-400 font-sans text-sm">No PYQs yet</p>
                      <button onClick={() => { setShowForm(true); setEditing(null); setForm(emptyForm) }} className="mt-3 text-xs font-mono text-amber-600 hover:text-amber-500">+ Add your first PYQ</button>
                    </td>
                  </tr>
                )}
                {Array.isArray(pyqs) && pyqs.map((pyq, i) => (
                  <tr key={pyq.id} className={`border-b border-stone-100 dark:border-stone-700/50 hover:bg-stone-50 dark:hover:bg-stone-700/30 transition-colors ${i % 2 === 0 ? '' : 'bg-stone-50/30 dark:bg-stone-800/30'}`}>
                    <td className="px-4 py-3.5">
                      <div className="font-medium text-stone-800 dark:text-stone-200">{pyq.title}</div>
                      {pyq.subjectName && <div className="text-xs text-stone-400 mt-0.5">{pyq.subjectName}</div>}
                    </td>
                    <td className="px-4 py-3.5 font-mono text-xs text-stone-500 uppercase">{pyq.fileType || 'pdf'}</td>
                    <td className="px-4 py-3.5">
                      <div className="flex gap-1.5">
                        {pyq.isFree && <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400">free</span>}
                        {pyq.isPremium && <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400">premium</span>}
                      </div>
                    </td>
                    <td className="px-4 py-3.5 font-mono text-sm font-semibold text-stone-700 dark:text-stone-300">{pyq.downloads || 0}</td>
                    <td className="px-4 py-3.5 text-right">
                      <button onClick={() => handleEdit(pyq)} className="px-3 py-1.5 text-xs font-mono text-amber-700 dark:text-amber-300 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-lg transition-all mr-1.5">Edit</button>
                      <button onClick={() => handleDelete(pyq.id, pyq.title)} className="px-3 py-1.5 text-xs font-mono text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {pyqs.length > 0 && (
        <div className="flex items-center justify-center gap-3 mt-5">
          ...
          <button onClick={() => setPage(p => p + 1)} disabled={pyqs.length < 20} className="px-4 py-2 bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded-xl text-sm font-mono text-stone-600 dark:text-stone-400 disabled:opacity-30 hover:bg-stone-50 dark:hover:bg-stone-700 transition-all disabled:cursor-not-allowed">
            Next →
          </button>
        </div>
      )}
